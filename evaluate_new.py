...
import numpy as np
from transformer.Translator_new import Translator
...
def main():
    ...
    with open(opt.output, 'w') as f:
        ...
        for (src,src_mask,src_lengths,trg,trg_y,trg_mask,trg_lengths) in tqdm(test_data, mininterval=2, desc='  - (Test)', leave=False):
#            src = torch.FloatTensor([[ idx2vec[int(idx.data.cpu().numpy())] for idx in src[i]] for i in  range(src.size(0))]).cuda(0)
            all_samples = translator.translate_batch(src,src_mask,src_lengths,trg,trg_mask,trg_lengths).data
            for bid in range(src.size(0)):
                numseq += 1.0

                ground_truth = np.zeros([numuser])
                num_ground_truth = 0
                for user in trg_y.data[bid][1:-1]:
...
#                    print(all_samples[0,0,0:1])
                    for j in range(num_ground_truth):
#                        print(bid,beid,j)
                        pred_uid=all_samples[bid,beid,j]
