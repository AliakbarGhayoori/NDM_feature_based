...
class Options(object):
    ...
    def __init__(self):
        ...
        # train file path.
        self.train_data = 'data/weibo3/cascade.txt'

        # test file path.
        self.test_data = 'data/weibo3/cascadetest.txt'

        self.u2vec_dict = 'data/weibo3/u2vec.pickle'

        self.idx2u_dict = 'data/weibo3/idx2u.pickle'

        self.user_data = 'data/weibo3/users_limited.txt'
        # save path.
        self.save_path = ''
...
class DataLoader(object):
    ...
    def __init__(
        ...
        if not load_dict:
            self._buildIndex()
            with open(self.options.u2vec_dict, 'wb') as handle:
                pickle.dump(self._u2vec, handle, protocol=pickle.HIGHEST_PROTOCOL)
            with open(self.options.idx2u_dict, 'wb') as handle:
                pickle.dump(self._idx2u, handle, protocol=pickle.HIGHEST_PROTOCOL)
        else:
            with open(self.options.u2vec_dict, 'rb') as handle:
                self._u2vec = pickle.load(handle)
            with open(self.options.idx2u_dict, 'rb') as handle:
                self._idx2u = pickle.load(handle)
            self.user_size = len(self._u2vec)
    ...
    def _buildIndex(self):
        ...
        pos = 0
        self._u2vec['<blank>'] = pos
        self._idx2u.append('<blank>')
        self.idx2vec[pos] = [pos]*160
        pos += 1
        self._u2vec['</s>'] = pos
        ...
        for user in user_set:
            self._u2vec[user] =  user_dic[user]
