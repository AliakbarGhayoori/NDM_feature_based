...
class Translator(object):
    ...
    def __init__(self, opt):
        ...
        h0 = torch.zeros(1, opt.beam_size, 8 * model_opt.d_word_vec)
    ...
    def translate_batch(self, batch, pred_len):
        ...
        user_size = self.model_opt.user_size
...
#            print(out.shape)
            ...
            sample = torch.multinomial(out.view(-1),beam_size,replacement=True)
            sample = [[x/user_size , x%user_size] for x in sample]
#            print(sample)
            new_sample = []
            for row,id in sample:
                new_smaple_row = torch.cat([dec_partial_seq[row],torch.LongTensor([id]).cuda(CUDA)],dim=0)
                new_sample.append(new_smaple_row)
            new_sample = torch.stack(new_sample)
