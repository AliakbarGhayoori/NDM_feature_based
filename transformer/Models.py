...
idx2vec_addr = "/media/external_3TB/3TB/ramezani/pmoini/Trial/data/idx2vec.pickle"
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.gru = nn.GRU(input_size=d_word_vec, hidden_size=160 * d_word_vec, num_layers=1, batch_first=True)

        self.dropout = nn.Dropout(dropout)
        self.conv = nn.Conv1d(160 * d_model, user_size, kernel_size, padding=kernel_size - 1, bias=True)
        #        self.conv = nn.Linear(4*d_model,user_size)
        self.padding = kernel_size - 1
        self.finit = finit

        self.layer_stack = nn.ModuleList([
            DecoderLayer(160 * d_model, d_inner_hid, n_head, d_k, d_v, dropout=dropout)
